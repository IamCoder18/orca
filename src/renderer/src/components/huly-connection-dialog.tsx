...
export function HulyConnectionDialog({
  ...
  const handleConnect = async (): Promise<void> => {
    ...
    if (
      ...
      setState((s) => ({ ...s, connectState: 'error', connectError: 'All fields are required.' }))
      return
    }
  ...
  const serverLabel = isRemote ? 'the Orca server' : 'this machine'
  const secretLabel = state.mode === 'token' ? 'Personal access token' : 'Password'
          ...
          <div className="grid gap-1">
            ...
            <div className="relative">
              <Input
                ...
                placeholder={state.mode === 'token' ? 'huly_pat_...' : 'password'}
                autoComplete="off"
              />
              <button
                type="button"
