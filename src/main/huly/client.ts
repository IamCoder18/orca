...
export async function connect(input: {
  ...
  // Why: the connect dialog validates credentials by running `huly whoami`
  // with env vars before persisting the secret. This catches typos and
  // missing CLI installs without storing anything.
  ...
  return {
    ok: true,
    connection,
    viewer: { displayName: input.email ?? input.name, email: input.email }
  }
}
